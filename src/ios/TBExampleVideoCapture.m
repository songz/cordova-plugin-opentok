...
-(id)initWithResolution: (NSString*) resolution{
    
    self = [super init];
    if (self) {
        _capturePreset = AVCaptureSessionPresetLow;
        NSArray *resParts = [resolution componentsSeparatedByString:@"x"];
        int resWidth = [resParts[0] integerValue];
        int resHeight = [resParts[1] integerValue];
        if (resWidth >= 640 && resHeight >= 480) {
            _capturePreset = AVCaptureSessionPresetHigh;
        }else if (resWidth >= 320 && resHeight >= 240){

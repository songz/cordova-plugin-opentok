...
public class CustomVideoCapturer extends BaseVideoCapturer implements
    ...
    public boolean setCameraIndex(String indexPos){
      int newIndex;

      if( compareStrings(indexPos, "front") ){
        newIndex = Camera.CameraInfo.CAMERA_FACING_FRONT;
      }else if( compareStrings(indexPos, "back") ){
        newIndex = Camera.CameraInfo.CAMERA_FACING_BACK;
      }else{
        newIndex = getFrontCameraIndex();
      }
      
      if(newIndex == this.mCameraIndex){
        return false;
      }else{
        this.mCameraIndex = newIndex;
        return true;
      }
    }
    public void setBackCamera(){
      this.swapCamera(Camera.CameraInfo.CAMERA_FACING_BACK);
    }
